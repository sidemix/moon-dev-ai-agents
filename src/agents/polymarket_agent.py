...
# ==============================================================================
# Polymarket Agent — FINAL FIXED VERSION
# ==============================================================================
        ...
        try:
            ...
            if isinstance(data, dict):
                if data.get('type') == 'subscribed':
                    cprint("WebSocket subscribed successfully to live trades!", "green")
                ...
                if topic == 'activity' and msg_type == 'orders_matched':
                    ...
                    if should_ignore:
                        if 'crypto' in _.lower():
                            self.ignored_crypto_count += 1
                        elif 'sports' in _.lower():
            ...
            cprint(f"Error processing WebSocket message: {e}", "yellow")
...
Provide predictions for each market in the specified format."""
        ...
        if USE_SWARM_MODE and self.swarm:
            ...
            successful_responses = [name for name, data in swarm_result.get('responses', {}).items() if data.get('success')]
            ...
            for model_name, model_data in swarm_result.get('responses', {}).items():
                if model_data.get('success'):
                    ...
                    cprint(f"\n{model_name.upper()} ({response_time:.1f}s)", "cyan", attrs=['bold'])
